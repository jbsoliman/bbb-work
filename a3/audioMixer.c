...
void AudioMixer_init(void)
{
	...
	// DONE:- Implement this. Hint: set the pSound pointer to NULL for each
	//     sound bite.
	for(int i = 0; i < MAX_SOUND_BITES; i++) {
		soundBites[i].pSound = NULL;
		soundBites[i].location = 0;
		//printf("The pointer for soundBites->pSound is: %p\n", soundBites[i].pSound);
	}
...
void AudioMixer_queueSound(wavedata_t *pSound)
{
	...
	if(queueFlag == false) {
		printf("ERROR: Did not queue sound\n");
	}
	//Add mutex unlock
...
static void fillPlaybackBuffer(short *playbackBuffer, int size)
{
	...
	int sbOffset;
	int sbTotalSamples;
	int buffAddToSoundBite;
	//Go through soundBites array
	for(int i = 0; i < MAX_SOUND_BITES; i++) {
		wavedata_t *tmpSound = soundBites[i].pSound;
		if(tmpSound != NULL) {
			//printf("Not null!\n");
			//Location of the soundBite buffer  
			sbOffset = soundBites[i].location;
			sbTotalSamples = soundBites[i].pSound->numSamples;
			//Go through buffer 
			for(int j = 0; j < size; j++) {
				//printf("Stuff in this buffer at %d\n", sbLoc);
				indexSum = (int) tmpSound->pData[sbOffset];
				buffAddToSoundBite = indexSum + playbackBuffer[j];
				if(sbTotalSamples >= sbOffset) {
					if (buffAddToSoundBite < CLAMP_MIN) {
						buffAddToSoundBite = CLAMP_MIN;
					} else if (buffAddToSoundBite > CLAMP_MAX) {
						buffAddToSoundBite = CLAMP_MAX;
					}
					//printf("indexSum value (after clamp): %d\n", indexSum);
					playbackBuffer[j] = (short) buffAddToSoundBite;
					sbOffset++;
				} else {
					soundBites[i].pSound = NULL;
					soundBites[i].location = 0;
				...
				//printf("indexSum value: %d\n", indexSum);
			}
			//printf("sbLoc value: %d\n", sbLoc);
			soundBites[i].location = sbOffset;
		}
	}
	pthread_mutex_unlock(&audioMutex);
}
